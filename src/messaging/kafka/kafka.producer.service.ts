...
export class KafkaProducerService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.warn(
        `⚠️ Kafka not available, skipping producer setup: ${error.message}`,
      );
    }
  }
  ...
  async emitBuildingCreatedEvent(payload: any) {
    try {
      this.logger.log(
        `📤 Emitting BUILDING_CREATED event: ${JSON.stringify(payload)}`,
      );
      ...
      this.logger.error(
        `❌ Failed to emit BUILDING_CREATED event: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async emitBuildingUpdatedEvent(payload: any) {
    try {
      this.logger.log(
        `📤 Emitting BUILDING_UPDATED event: ${JSON.stringify(payload)}`,
      );
      ...
      this.logger.error(
        `❌ Failed to emit BUILDING_UPDATED event: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async emitBuildingDeletedEvent(payload: any) {
    try {
      this.logger.log(
        `📤 Emitting BUILDING_DELETED event: ${JSON.stringify(payload)}`,
      );
      ...
      this.logger.error(
        `❌ Failed to emit BUILDING_DELETED event: ${error.message}`,
      );
      throw error;
    }
  }
